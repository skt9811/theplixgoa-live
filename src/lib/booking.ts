...
// (Neon + Resend, see confirm-booking.server-fn.ts). This is the fast,
// client-triggered path — the Razorpay webhook is the reliability backstop
// that fires independently of this; confirmBookingAndSendEmails's own
// atomic confirmation_sent_at guard is what keeps the two from double-
// sending, not anything here. Payment already succeeded and the booking row
// is already updated by this point (updateBookingPayment, called before
// this) — an email failure here must never block or roll back that, so
