...
// widget calls back with a success), so unlike the webhook — which
// independently verifies the whole request body's HMAC before ever getting
// here — this file is the one place responsible for verifying the
// checkout-success signature before marking anything paid or emailing a
// confirmation. See razorpay-verify.server.ts for why that check must not
// be skipped.
