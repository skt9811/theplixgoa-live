...
// Client-side booking voucher generator — runs entirely in the browser, no
// server involved. Ported from the (undeployed) supabase/functions/_shared
// edge-function version; same layout, same lesson learned there: never use
// Intl's style:"currency" for INR in a pdf-lib document — the ₹ glyph isn't
// in the standard fonts' WinAnsi encoding and throws on every draw call.
...
// Deliberately not style:"currency" — see file header note.
...
/** Generates the voucher and triggers a browser download — no server round-trip. */
export async function downloadVoucherPdf(booking: VoucherBooking): Promise<void> {
  const bytes = await generateVoucherPdf(booking);
  const blob = new Blob([new Uint8Array(bytes)], { type: "application/pdf" });
  const url = URL.createObjectURL(blob);
  try {
    const a = document.createElement("a");
    a.href = url;
    a.download = `ThePlixGoa_Voucher_${booking.id.slice(0, 8).toUpperCase()}.pdf`;
