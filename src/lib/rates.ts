...
import type { RateOverride } from "@/lib/rates-utils";

// Pure pricing/date helpers live in rates-utils.ts (no Supabase dependency) so
// components that only need this math don't pull the Supabase SDK into their
// bundle. Re-exported here for existing consumers of "@/lib/rates".
export * from "@/lib/rates-utils";
...
export async function fetchTodayRate(propertyId: string, basePrice: number): Promise<number> {
  ...
  const overrides = await fetchRateOverrides(propertyId, todayStr, todayStr);
  return overrides[todayStr] ?? basePrice;
}
